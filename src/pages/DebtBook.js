...
function DebtBook() {
    ...
    // State to hold all debtBook
    const [debtBook, setDebtBook] = useState([]);
    ...
    const generateId = () => debtBook.length + 1;
    ...
    const handleSubmit = (e) => {
        ...
        if (editIndex !== null) {
            ...
            const updateDebtBook = debtBook.map((expense, index) =>
                index === editIndex ? { ...formData, id: expense.id, date: expense.date } : expense
            );
            setDebtBook(updateDebtBook);
        } else {
            ...
            setDebtBook([...debtBook, newDebtBook]);
        }
    ...
    const handleEdit = (index) => {
        setFormData(debtBook[index]);
        setEditIndex(index);
        setShowModal(true);
    };

    // Filter debtBook based on the search query
    const filtereddebtBook = debtBook.filter((expense) =>
        expense.name.toLowerCase().includes(searchQuery.toLowerCase())
    );

    return (
        <div className="layout">
            <h1 className="text-2xl font-bold mb-4 text-center">دفتر الدين</h1>
            ...
            <h2 className="text-2xl font-bold mb-4 text-center"> قائمة دين</h2>
            {/* Search Bar */}
            <input
                type="text"
            ...
            <table className="w-full table-fixed bg-white shadow-md rounded">
                ...
                <tbody>
                    {filtereddebtBook.length > 0 ? (
                        filtereddebtBook.map((expense, index) => (
                            <tr key={expense.id} className="border-b hover:bg-gray-100">
                                ...
                                <td className="px-4 py-2">
                                    ...
                                    <button
                                        onClick={() => setDebtBook(debtBook.filter((_, i) => i !== index))}
                ...
                <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center">
                    <div className="bg-white p-6 rounded shadow-lg w-96">
                        <h2 className="text-lg font-bold mb-4">
                            {editIndex !== null ? 'Edit Expense' : (<h2 className="text-lg font-bold mb-4"> اضافة دين</h2>)}
                        </h2>
